...
A simple set of racing training tools using Simhub overlays. Add these overlays to [Simhub](https://www.simhubdash.com/) and you can use them for all simhub-supported racing titles.
...
An overlay that shows the current position of the clutch, brake and accelerator inputs. It can be used to analyse pedal inputs and improve driving technique.

## Steering Wheel
![Wheel Tracer](https://github.com/user0451/Simhub-Telemetry-Overlays/blob/main/media/wheel.gif)

An overlay that shows the current rotation of the steering wheel.

Modelled on the Moza CS2 Pro and the FSR2 wheels, but you can **swap the image to any other wheel** if you want to.

>The dashboard on the wheels are not really intended to be used when driving, it is not 'a feature'. I was just covering the static dashboard in the image of the wheels I stole from the Moza website and got carried away! I liked the idea of having a working dashboard in a wheel that will be displayed so small, you can't even see the dashboard...
...
> - scroll the list to find your steering wheel X axis (try moving your wheel to find the changing value)
> - Once you have found your wheel axis, turn your wheel full lock to the right (without a game loaded). Copy the value shown here and set in **Direct Max** if very different from the default (65535)
...
The Pedal tracer overlay works slightly differently. Our variables are not attached to the dashboard this time, but instead are attached to some widgets on the overlay.
...
|----------|-------------|---------|
| **accel** | The Simhub property value representing your accelerator axis | [JoystickPlugin.MOZA_R5_Base_Z] |
| **brake** | The Simhub property value representing your brake axis | [JoystickPlugin.MOZA_R5_Base_RZ] |
| **clutch** | The Simhub property value representing your clutch axis | [JoystickPlugin.MOZA_R5_Base_Y] |
...
Game Input Mode does not require any variables to be set, as it uses the game telemetry to report the pedal inputs.
...
### Which mode am I in?
We can show the mode in a label if you like.

To show you which mode you are in, both overlays need to know which button you have just set to switch modes for that overlay. So, we need to add one more variable value to the dashboard variables for each overlay.
...
Once you have added unique keys to this variable on each of the overlays, save the overlay and you will see a label appear on the overlay when you switch modes. The label will show the current mode for 2 seconds or so, as you switch and then disappear.

## The cost...
Well, the overlays are free, but will cost a little bit of patience and understanding to customise them to your specific hardware. Easy for some, a little more difficult for others. I have tried to make the Simhub overlays as easy to customise as possible.

> ***SAVE***
> 
> **Remember to save your changes when adding your values to the overlays, or they will not apply.**

### Forget about it...
All but one of the values you will need to customise are 'fire and forget'; do it once and then forget about them. The **GAME_Steering_Angle** variable should be changed to match the vehicle you are currently driving, only if you want to use the **Game Input mode** on the Steering Wheel.
...
The sims I use are AC and AMS2. Both of which automatically adjust the steering lock (Degrees of Rotation) per car, so they force a soft-lock. When I'm in a little cart in AMS2, I can only turn my steering wheel 180°, or 300-360° in a formula car, despite having 1080° set in Moza Pithouse. This is the games soft-lock working. Your game may manage this automatically, or require you to change some setting in game.

> The **direct input** mode of the steering wheel overlay will always match the DoR produced by the soft-lock of your current vehicle (as long as you have set **DIRECT_Steering_Angle** correctly...).
